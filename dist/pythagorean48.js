 ...
 */
const DIRECTION_VECTORS = Array.from({ length: DIRECTION_COUNT }, (_, k) => {
 ...
 */
export function trustToDirection(trust) {
    // Clamp to [-1, 1]
 ...
 */
export function directionToTrust(dir) {
 ...
 */
export function trustVector(dir) {
 ...
 */
export function trustSimilarity(trustA, trustB) {
    ...
    if (Math.abs(dirA - dirB) === DIRECTION_COUNT / 2)
        return -1;
    // Otherwise, compute angular distance
    const minDiff = Math.min(Math.abs(dirA - dirB), DIRECTION_COUNT - Math.abs(dirA - dirB));
 ...
 */
export function vectorDot(vecA, vecB) {
    let sum = 0;
